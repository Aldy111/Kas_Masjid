	...
	<div class="container">
		<div class="row">
			<div class="col-8 mx-auto text-center">
				<h2 class="mb-3 text-capitalize">Kegiatan</h2>
				<ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
					<li class="list-inline-item"><a href="index.html">Home</a>
					</li>
					<li class="list-inline-item">/ &nbsp; <a href="blog.html">Kegiatan</a>
					</li>
				</ul>
			</div>
		</div>
                    ...
                    @foreach($kegiatan as $row)
						<div class="col-md-4" data-aos="fade">
							<article class="blog-post">
								<div class="post-slider slider-sm rounded h-25">
                                @empty($row->foto)
                                    <img loading="lazy" decoding="async" src="{{ url('admin/img/nophoto.png') }}" alt="Post Thumbnail" >
                                @else
                                    <img loading="lazy" decoding="async" src="{{ url('admin/img')}}/{{$row->foto}}" alt="Post Thumbnail">
                                @endempty
								</div>
								<div class="pt-4">
									<p class="mb-3">{{\Carbon\Carbon::parse($row->tgl_kegiatan)->translatedFormat('l,d F Y')}}</p>
									<h2 class="h4"><a class="text-black" href="{{ route('kegiatan2.show',$row->id) }}">{{$row->judul_kegiatan}}</a></h2>
									<p>{{$row->ket}}</p> <a href="{{ route('kegiatan2.show',$row->id) }}" class="text-primary fw-bold" aria-label="Read the full article by clicking here">Read More</a>
								</div>
							</article>
						</div>
                    @endforeach
							...
							<nav class="mt-4">
								...
								<nav class="mb-md-50">
									<ul class="pagination justify-content-center">
										<li class="page-item active "> <a href="{{ url('/kegiatan2') }}" class="page-link">1</a>
										</li>
										<li class="page-item"> <a href="{{ url('/kegiatan2') }}" class="page-link">2</a>
										</li>
										<li class="page-item">
											<a class="page-link" href="{{ url('/kegiatan2') }}" aria-label="Pagination Arrow"> <i class="fas fa-angle-right"></i>
											</a>
										</li>
									</ul>
								</nav>
