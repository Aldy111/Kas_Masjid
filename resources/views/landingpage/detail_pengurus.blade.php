	...
	<div class="container">
		<div class="row">
			<div class="col-8 mx-auto text-center">
				<h2 class="mb-3 text-capitalize">Detail Pengurus</h2>
				<ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
					<li class="list-inline-item"><a href="{{ url('/home')}}">Home</a>
					</li>
					<li class="list-inline-item">/ &nbsp; <a href="{{ url('/pengurus')}}">Kepengurusan</a>
					</li>
				</ul>
			</div>
		</div>
		...
		<div class="row g-5">
			<div class="col-lg-4 mb-5 mb-lg-0">
				<div class="bg-white shadow rounded-lg p-4 sticky-top" style="top: 30px;">
								@empty($row->foto)
                                    <img loading="lazy" decoding="async" src="{{ url('admin/img/nophoto.png') }}" alt="Scarlet Pena" class="rounded w-100" width="300" height="332" >
                                @else
                                    <img loading="lazy" decoding="async" src="{{ url('admin/img')}}/{{$row->foto}}"alt="Scarlet Pena" class="rounded w-100" width="300" height="332">
                                @endempty
								<br>
								<br>
								<h4 class="card bg-transparent border-0 text-center">{{$row->nama}}</h4>
				</div>
			</div>
			<div class="col-lg-8">
				<div class="content">
					...
					<ul>
						<li>Nama : {{$row->nama}} </li>
						<li>Kode Petugas : {{$row->kode_petugas}} </li>
						<li>Jabatan : {{$row->jabatan->nama_jabatan}} </li>
						<li>Tempat Lahir : {{$row->tmp_lahir}} </li>
						<li>Tanggal Lahir : {{\Carbon\Carbon::parse($row->tgl_lahir)->translatedFormat('l,d F Y')}} </li>
						<li>Jenis Kelamin : {{$row->gender}} </li>
						<li>Status : {{$row->status}} </li>
						<li>No Handphone : {{$row->no_hp}} </li>
